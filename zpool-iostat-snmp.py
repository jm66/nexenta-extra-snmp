    ...
    with open(ZPOOLS_IOSTAT_CACHE_FILE) as cache_file:
        ...
        for zpool in zpools_iostat:
            ...
            for c_device in zpool.cache:
                ...
                pp.add_str('2.3.10.1.' + c_oid, c_device.label)
                pp.add_gau('2.3.10.2.' + c_oid, c_device.oread)
                pp.add_gau('2.3.10.3.' + c_oid, c_device.owrite)
                pp.add_gau('2.3.10.4.' + c_oid, c_device.bread)
                pp.add_gau('2.3.10.5.' + c_oid, c_device.bwrite)
                pp.add_gau('2.3.10.6.' + c_oid, c_device.lread)
                pp.add_gau('2.3.10.7.' + c_oid, c_device.lwrite)
            for l_device in zpool.log:
                l_oid = pp.encode(l_device.label)
                pp.add_str('2.3.11.1.' + l_oid, c_device.label)
                pp.add_gau('2.3.11.2.' + l_oid, c_device.oread)
                pp.add_gau('2.3.11.3.' + l_oid, c_device.owrite)
                pp.add_gau('2.3.11.4.' + l_oid, c_device.bread)
                pp.add_gau('2.3.11.5.' + l_oid, c_device.bwrite)
                pp.add_gau('2.3.11.6.' + l_oid, c_device.lread)
                pp.add_gau('2.3.11.7.' + l_oid, c_device.lwrite)
