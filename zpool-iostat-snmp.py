    ...
    with open(ZPOOLS_IOSTAT_CACHE_FILE) as cache_file:
        ...
        for zpool in zpools_iostat:
            ...
            for device in zpool.devices:
                pp.add_str('2.3.10.1.' + oid, device.label)
                pp.add_gau('2.3.10.2.' + oid, device.oread)
                pp.add_gau('2.3.10.3.' + oid, device.owrite)
                pp.add_gau('2.3.10.4.' + oid, device.bread)
                pp.add_gau('2.3.10.5.' + oid, device.bwrite)
                pp.add_gau('2.3.10.6.' + oid, device.lread)
                pp.add_gau('2.3.10.7.' + oid, device.lwrite)
            for l_device in zpool.log:
                pp.add_str('2.3.11.1.' + oid, l_device.label)
                pp.add_gau('2.3.11.2.' + oid, l_device.oread)
                pp.add_gau('2.3.11.3.' + oid, l_device.owrite)
                pp.add_gau('2.3.11.4.' + oid, l_device.bread)
                pp.add_gau('2.3.11.5.' + oid, l_device.bwrite)
                pp.add_gau('2.3.11.6.' + oid, l_device.lread)
                pp.add_gau('2.3.11.7.' + oid, l_device.lwrite)
            for c_device in zpool.cache:
                pp.add_str('2.3.12.1.' + oid, c_device.label)
                pp.add_gau('2.3.12.2.' + oid, c_device.oread)
                pp.add_gau('2.3.12.3.' + oid, c_device.owrite)
                pp.add_gau('2.3.12.4.' + oid, c_device.bread)
                pp.add_gau('2.3.12.5.' + oid, c_device.bwrite)
                pp.add_gau('2.3.12.6.' + oid, c_device.lread)
                pp.add_gau('2.3.12.7.' + oid, c_device.lwrite)
