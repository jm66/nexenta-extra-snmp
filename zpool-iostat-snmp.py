    ...
    with open(ZPOOLS_IOSTAT_CACHE_FILE) as cache_file:
        ...
        for zpool in zpools_iostat:
            ...
            for l_device in zpool.log:
                ...
                pp.add_str('2.3.11.1.' + l_oid, l_device.label)
                pp.add_gau('2.3.11.2.' + l_oid, l_device.oread)
                pp.add_gau('2.3.11.3.' + l_oid, l_device.owrite)
                pp.add_gau('2.3.11.4.' + l_oid, l_device.bread)
                pp.add_gau('2.3.11.5.' + l_oid, l_device.bwrite)
                pp.add_gau('2.3.11.6.' + l_oid, l_device.lread)
                pp.add_gau('2.3.11.7.' + l_oid, l_device.lwrite)
            for c_device in zpool.cache:
                c_oid = pp.encode(c_device.label)
                pp.add_str('2.3.12.1.' + c_oid, c_device.label)
                pp.add_gau('2.3.12.2.' + c_oid, c_device.oread)
                pp.add_gau('2.3.12.3.' + c_oid, c_device.owrite)
                pp.add_gau('2.3.12.4.' + c_oid, c_device.bread)
                pp.add_gau('2.3.12.5.' + c_oid, c_device.bwrite)
                pp.add_gau('2.3.12.6.' + c_oid, c_device.lread)
                pp.add_gau('2.3.12.7.' + c_oid, c_device.lwrite)
